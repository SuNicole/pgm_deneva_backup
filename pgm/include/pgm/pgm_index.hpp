...
public:
    ...
    auto init(const std::vector<K> &data){
        // PGMIndex(data.begin(), data.end());
        n = std::distance(data.begin(), data.end());
        first_key = n ? *data.begin() : K(0);
        build(data.begin(),data.end(), Epsilon, EpsilonRecursive, segments, levels_offsets);
    }

    void copy_to_buf(char * buf,uint64_t &size){
        printf("*********n*** = %d\n",n);
        memcpy(buf,&n,sizeof(size_t));

        size_t tmp = *(size_t *)(buf);
        printf("[pgm_index.hpp:205]tmp = %d, %d\n",tmp,*(size_t *)buf);

        uint64_t offset = sizeof(size_t);
        memcpy(buf + offset,&first_key,sizeof(K));
        K tmp2 = *(K *)(buf+sizeof(size_t));
        printf("[pgm_index.hpp:210]tmp = %d, %d\n",tmp2,*(K *)(buf+sizeof(size_t)));

        uint64_t segments_size = segments.size();
        offset = offset + sizeof(K);
        memcpy(buf + offset,&segments_size,sizeof(uint64_t));

        offset = offset+sizeof(uint64_t);
        for(int i = 0;i < segments.size();i++){
            memcpy(buf + offset,&segments[i],sizeof(Segment));
            offset = offset + sizeof(Segment);
        }

        uint64_t levels_offsets_size = levels_offsets.size();
        memcpy(buf + offset,&levels_offsets_size,sizeof(uint64_t));

        offset = offset+sizeof(uint64_t);
        for(int i = 0;i < levels_offsets.size();i++){
            memcpy(buf + offset,&levels_offsets[i],sizeof(size_t));
...
/*****/
    ...
    K get_first_key(){return first_key;}

    std::vector<Segment> get_segment(){
        // printf("[pgm_index.hpp:254]segment size = %ld\n",segments.size());
        return segments; }

    std::vector<size_t> get_level_offset(){return levels_offsets;}

    void set_n(size_t num){n = num;}

    void set_first_key(K num){ first_key = num;}
    
    void set_segment(std::vector<Segment> tmp_seg){
        printf("[pgm_index.hpp:262]segment size = %ld\n",segments.size());
        segments.assign(tmp_seg.begin(),tmp_seg.end());  
    }

    void set_level_offset(std::vector<size_t> tmp_level){
        levels_offsets.assign(tmp_level.begin(),tmp_level.end());  
    }
/*****/
