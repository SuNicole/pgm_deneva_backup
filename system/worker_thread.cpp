...
RC WorkerThread::process_rtxn( yield_func_t &yield, Message * msg, uint64_t cor_id) {
  ...
  if(msg->get_rtype() == CL_QRY || msg->get_rtype() == CL_QRY_O) {
    ...
    INC_STATS(get_thd_id(),worker_activate_txn_time,get_sys_clock() - ready_starttime);
    assert(ready);
    if (CC_ALG == WAIT_DIE || CC_ALG == RDMA_WAIT_DIE2 || CC_ALG == RDMA_WOUND_WAIT2 || CC_ALG == RDMA_WAIT_DIE || CC_ALG == RDMA_WOUND_WAIT || CC_ALG == RDMA_MOCC|| CC_ALG == RDMA_OPT_WAIT_DIE) {
  ...
  if(msg->get_rtype() == CL_QRY || msg->get_rtype() == CL_QRY_O) {
    ...
    INC_STATS(get_thd_id(),worker_activate_txn_time,get_sys_clock() - ready_starttime);
    assert(ready);
    if (CC_ALG == WAIT_DIE || CC_ALG == RDMA_WAIT_DIE2 || CC_ALG == RDMA_WOUND_WAIT2 || CC_ALG == WOUND_WAIT || CC_ALG == RDMA_WAIT_DIE || CC_ALG == RDMA_WOUND_WAIT || CC_ALG == RDMA_MOCC|| CC_ALG == RDMA_OPT_WAIT_DIE) {
      #if WORKLOAD == DA //mvcc use timestamp
